...
int ReceivePoint(igtl::Socket * socket, igtl::MessageHeader * header)
{
  ...
  if (c & igtl::MessageHeader::UNPACK_BODY) // if CRC check is OK
    ...
    for (int i = 0; i < nElements; i ++)
	  ...
	  igtlFloat32 rempos[3];
	  pointElement->GetPosition(rempos);
	  pointElement->SetPosition(rempos[0]*-1, rempos[1]*-1, rempos[2]*-1);
	  
	  pointMsgToSend->AddPointElement(pointElement);	
	  pointMsgToSend->Pack();
